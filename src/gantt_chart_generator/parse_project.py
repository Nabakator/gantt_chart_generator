...
def _parse_project(data: Any, path: _Path) -> Project:
    ...
    if categories_raw is None:
        raise ProjectValidationError(f"{path}: missing required field 'categories'")
    if not isinstance(categories_raw, list):
        raise ProjectValidationError(f"{path}.categories: expected list")
...
def _parse_category(data: Any, path: _Path, ids: set[str]) -> Category:
    if not isinstance(data, dict):
        raise ProjectValidationError(f"{path}: expected mapping for category")
    ...
    if color is not None and not isinstance(color, str):
        raise ProjectValidationError(f"{path}.color: expected string")
...
def _parse_item(data: Any, path: _Path, ids: set[str]) -> ProjectItem:
    if not isinstance(data, dict):
        raise ProjectValidationError(f"{path}: expected mapping for item")

    node_type = data.get("type")
    if node_type is None:
        raise ProjectValidationError(f"{path}: missing required field 'type'")
    if node_type not in {"group", "workpackage", "milestone"}:
        raise ProjectValidationError(f"{path}.type: unknown type '{node_type}'")
...
def _parse_group(data: dict[str, Any], path: _Path, ids: set[str]) -> Group:
    ...
    if category is not None and not isinstance(category, str):
        raise ProjectValidationError(f"{path}.category: expected string")
...
def _parse_work_package(data: dict[str, Any], path: _Path, ids: set[str]) -> WorkPackage:
    ...
    if not isinstance(depends_on_raw, list):
        raise ProjectValidationError(f"{path}.depends_on: expected list of ids")
    depends_on: list[str] = []
    for idx, dep in enumerate(depends_on_raw):
        if not isinstance(dep, str):
            raise ProjectValidationError(f"{path}.depends_on[{idx}]: expected string id")
...
def _require_str(data: dict[str, Any], key: str, path: _Path) -> str:
    ...
    if not isinstance(value, str):
        raise ProjectValidationError(f"{path.child(key)}: expected string")
    return value
...
def _register_id(value: str, path: _Path, ids: set[str]) -> None:
    if value in ids:
        raise ProjectValidationError(f"{path}: duplicate id '{value}'")
    ids.add(value)
